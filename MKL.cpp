...
void MKL::linear_solver_svc()
{
	...
	if(param->solver_type == L1LOSS_SVM_DUAL)
	{
		diag_p = 0; diag_n = 0;
...
void MKL::mkl_svm_train(double *w, double* alpha_, double *d_sigma)
{
	...
	for(p=0; p< num_kernel; p++)
	{
		if(d_sigma[p] != 0)
		{
			...
			{	
				tmp += w[param->nB*p+i]*w[param->nB*p+i];					
			}
			v += tmp*d_sigma[p];
		}
	}
...
void MKL::SimpleMKL()
{
	...
	for(i=0; i<n_kernel; i++)
	{
		...
		{
			grad[i] += model_->w[i*param->nB+j]*model_->w[i*param->nB+j];
		}
		grad[i] *= -0.5;
	}
	...
	while(loop==1 && maxloop > 0 && n_kernel > 1)
	{
		...
		if(flag == 1)
		{
			...
			double *tmp_w = Malloc(double, (param->nB+1)*n_kernel);
			...
			double *tmp_w_1 = Malloc(double, (param->nB+1)*n_kernel);
			double *tmp_alpha_1 = Malloc(double, l+1);

			double *tmp_sigma_2 = Malloc(double, n_kernel);
			double *tmp_w_2 = Malloc(double, (param->nB+1)*n_kernel);
